...
type APIController struct {
	...
	spaController     *SpaController // Anti-DPI
	settingService    *service.SettingService
	Tgbot             service.Tgbot
}

func NewAPIController(g *gin.RouterGroup, s *ServerController, settingService *service.SettingService) *APIController {
	a := &APIController{
		serverController: s,
...
func (a *APIController) initRouter(g *gin.RouterGroup) {
	...
	a.inboundApi(api)
	a.serverApi(api)
	a.antiDpiApi(api) // Anti-DPI routes
}
...
// antiDpiApi مسیرهای REST مربوط به Anti-DPI SPA را ثبت می‌کند
func (a *APIController) antiDpiApi(api *gin.RouterGroup) {
	var spaSvc SpaService
	if raw := global.GetSpaService(); raw != nil {
		spaSvc = raw
	}
	a.spaController = NewSpaController(api, spaSvc, a.settingService)
}
